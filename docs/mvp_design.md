...
系统当前支持 Python 与 Java 后端项目的项目画像、风险面识别、工具规划和安全审计流程。后续可通过扩展语言识别规则、漏洞知识库标签和工具注册表继续支持 JavaScript、TypeScript、Go 等项目。
...
- Semgrep：作为注册工具接入，用于多语言规则扫描。
- Bandit：作为 Python 安全扫描工具接入。
- Gitleaks：作为 secret 泄露扫描工具接入。
- SARIF：输出标准静态分析报告，便于接入 GitHub Code Scanning 或安全平台。
- GitHub Action：在 push、PR 或定时任务中触发审计流程。
- GitHub PR Comment：将 PR 审计摘要、风险数量和报告链接回写到代码评审流程。
- MCP Adapter：将外部 MCP Tool Server 暴露的工具映射为项目内部 SecurityTool，再由 ToolExecutor 统一调用。
...
- `risk_planner`：根据项目画像和知识库规划审计重点。
- `llm_tool_calling_auditor`：提出受控工具调用，补充上下文或验证风险假设。
- `llm_supplemental_auditor`：基于工具观察和源码上下文补充 finding。
- `risk_analyzer`：解释风险原因、攻击场景、严重程度和置信度。
- `false_positive_reviewer`：复核误报。
- `fix_advisor`：生成修复建议。
...
- SARIF 用于接入标准代码扫描平台。
- GitHub Action / PR Comment 用于 PR 级自动安全审查。
...
`RiskPlan` 会交给全局 `tool_selector` 校验和转换，形成真正可执行的 `ToolPlan`。
...
`context_extractor` 为每个 finding 提取证据。
...
系统识别 FastAPI、SQLAlchemy、路由文件、认证文件和数据库访问文件，命中 SQL Injection、Secret Leak、Broken Access Control 知识条目，选择 secret scanner、自定义规则、Semgrep 和 Bandit，并由 LLM 补充检查路由到数据库访问的风险链路。

### Java 后端项目

系统识别 Spring Boot、Controller、Service、Mapper、MyBatis XML 和 Spring Security 配置，命中 SQL Injection、Broken Access Control、Command Injection 知识条目，选择 Semgrep、自定义规则和上下文提取工具，并由 LLM 检查 Controller 到 Service / Mapper 的调用关系。

### Git diff 审计

系统解析 changed files、hunks 和新增行，只围绕本次变更生成局部项目画像、匹配漏洞知识、选择工具和执行审计。报告会标记 finding 是否来自 diff 新增行，适合接入 PR 审查流程。
